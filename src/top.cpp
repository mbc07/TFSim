...
void top::rob_mode(unsigned int nadd, unsigned int nmul,unsigned int nload,unsigned int nbpb,unsigned int nbtb,map<string,int> instruct_time, vector<string> instruct_queue, nana::listbox &table, nana::grid &mem_gui, nana::listbox &regs, nana::listbox &instr_gui, nana::label &ccount, nana::listbox &rob_gui)
{
    ...
    rob = unique_ptr<reorder_buffer>(new reorder_buffer("rob",rob_size,2,nbpb,nbtb,rob_gui,instr_gui.at(0)));
